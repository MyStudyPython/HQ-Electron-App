  ...
  return {
    ...
    closeBundle() {
      ...
      if (platform === 'win32') {
        ...
        // windows 上需要执行这种方式
        spawn('npm.cmd', ['run', 'make'], { stdio: 'inherit' }) as ChildProcess
      } else if (platform === 'darwin') {
        console.log('当前运行环境是 Mac')
        // Mac上可以执行这种方式
        // spawn('electron-forge',['make'],{stdio: 'inherit'}) as ChildProcess
        spawn('npm', ['run', 'make'], { stdio: 'inherit' }) as ChildProcess
      } else {
        console.error('其他平台 : ', platform, '【暂不支持打包】')
      }
    },
  }
}
